...
def _to_list(value):
    if isinstance(value, torch.Tensor):
        return value[0].detach().cpu().tolist() if value.ndim > 1 else value.detach().cpu().tolist()
    if isinstance(value, (list, tuple)):
        return list(value)
    return None


def _resolve_action_scale(action_term) -> list[float] | None:
    """Resolve per-joint action scale across different action term implementations."""
    # Standard JointPositionAction path.
    scale = _to_list(getattr(action_term, "_scale", None))
    if scale is not None:
        return scale

    # Wrapper action terms (e.g. ResidualLatentAction) usually hold a nested joint action term.
    nested_joint_action = getattr(action_term, "_joint_action", None)
    if nested_joint_action is not None:
        scale = _to_list(getattr(nested_joint_action, "_scale", None))
        if scale is not None:
            return scale

    # Final fallback from config if available (can be scalar or sequence).
    cfg_scale = getattr(getattr(action_term, "cfg", None), "scale", None)
    if isinstance(cfg_scale, (int, float)):
        action_dim = int(getattr(action_term, "action_dim", 1))
        return [float(cfg_scale)] * action_dim
    scale = _to_list(cfg_scale)
    if scale is not None:
...
def attach_onnx_metadata(env: ManagerBasedRLEnv, run_path: str, path: str, filename="policy.onnx") -> None:
    ...
    metadata = {
        ...
        "action_scale": _resolve_action_scale(joint_pos_term),
