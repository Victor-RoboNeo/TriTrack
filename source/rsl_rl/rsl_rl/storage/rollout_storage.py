...
class RolloutStorage:
    class Transition:
        def __init__(self):
            ...
            # For AnyBody latent distillation: teacher latent (mu)
    ...
    def __init__(
        ...
        self.teacher_latent_shape = teacher_latent_shape
        ...
        # for distillation, PULSE distillation, and latent-consistent distillation
        if training_type in (
            "distillation",
            "pulse_distillation",
        ...
        # for AnyBody latent distillation (teacher latent + privileged_actions)
        if training_type == "anybody_latent_distillation":
            self.privileged_actions = torch.zeros(num_transitions_per_env, num_envs, *actions_shape, device=self.device)
            if teacher_latent_shape is not None:
                self.teacher_latent = torch.zeros(
                    num_transitions_per_env, num_envs, *teacher_latent_shape, device=self.device
                )
            else:
                self.teacher_latent = None
    ...
    def add_transitions(self, transition: Transition):
        ...
        # for AnyBody latent distillation
        if self.training_type == "anybody_latent_distillation":
            self.privileged_actions[self.step].copy_(transition.privileged_actions)
            if self.teacher_latent is not None and transition.teacher_latent is not None:
                self.teacher_latent[self.step].copy_(transition.teacher_latent)
    ...
    # for distillation, PULSE distillation, latent-consistent distillation, and anybody_latent_distillation
    def generator(self):
        ...
        for i in range(self.num_transitions_per_env):
            ...
            if self.training_type == "anybody_latent_distillation" and self.teacher_latent is not None:
                yield (
                    self.observations[i],
