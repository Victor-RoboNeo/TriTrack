#!/usr/bin/env python3
"""WORLD live-anchor eval for FLAT_UNIFIED_SPARSE_INTENT_V1.

No 18-D task error. No terrain. known_preview lookahead (parent contract).
"""
...
ap.add_argument("--max-traj-per-family", type=int, default=12)
...
ap.add_argument("--mask", default="1,1,1", help="H,LH,RH active bits")
...
def select_items(items, max_per_family: int):
    by_f: dict[str, list] = {}
    for w, r in items:
        by_f.setdefault(r.get("family", "unk"), []).append((w, r))
    out = []
...
def main() -> None:
    ...
    items = select_items(load_suite(Path(args_cli.suite)), args_cli.max_traj_per_family)
    n_env = len(items)
    if n_env == 0:
    ...
    runner = OnPolicyRunner(env, agent_cfg.to_dict(), log_dir="/tmp/fusi_eval", device=agent_cfg.device)
    device = env.device
