#!/usr/bin/env python3
"""Step 6S-T — Live-trigger paired 0.5 s counterfactual transfer. No PPO. No CL2.

Frozen: Mapper-B + model_50000 + 6S MLP. Step-3 gate. Loco torso.

At each Parent detect (R_E>=1 persist-3), clone x_t and branch:

  P: Parent 0.5 s
  R: 5° / 100 ms burst, then Parent to 0.5 s

I_h = E^R_{t+h} - E^P_{t+h}  at 100 / 260 / 500 ms  (5 / 13 / 25 steps).
Δt = 0 by construction. GO metric is P(I_500 < 0), not absolute ΔE@100ms.
"""
...
parser = argparse.ArgumentParser(description="P2-R Step 6S-T live-trigger paired transfer.")
...
parser.add_argument("--out", type=str, default="results/p2r_step6s_t")
...
parser.add_argument("--max_states", type=int, default=80)
...
def _apply_terrain(env_cfg, terrain: str) -> None:
    if terrain == "plane":
        ...
        print("[6s-t] terrain=plane", flush=True)
        return
    ...
    print(f"[6s-t] terrain={terrain} 1x1 generator", flush=True)
    ...
    with torch.inference_mode():
        ...
        if mask_name and hasattr(cmd, "set_eval_fixed_mask_mode_idx"):
            ...
            if mask_name in names:
                ...
                print(f"[6s-t] pin mask {mask_name!r} idx={names.index(mask_name)}", flush=True)
        ...
        try:
            ...
            print(f"[6s-t] sim.forward skipped: {exc}", flush=True)
...
def _collect_triggers(
    ...
    *,
    ...
    for t in range(steps):
        if remain > 0 and len(snaps) >= remain:
            break
        with torch.inference_mode():
            ...
            e9_prev = e9.detach()
            active_prev = active
    print(f"[6s-t] collected {len(snaps)} triggers terrain={terrain} seed={seed}", flush=True)
    return snaps
...
def _summarize_rows(rows: list[dict], nest: bool = True) -> dict:
    ...
    abs100 = np.array([r["dE_R_100_cm"] for r in rows], dtype=np.float64)
    out["abs_dE_R_100"] = _i_block(abs100)
    out["abs_dE_R_100"]["note"] = "diagnostic only: E^R_{t+0.1}-E_t, not GO"
    if nest:
        terrains_present = sorted({r["terrain"] for r in rows})
        if len(terrains_present) > 1:
            out["by_terrain"] = {
                ter: _summarize_rows([r for r in rows if r["terrain"] == ter], nest=False)
                for ter in terrains_present
            }
        first = [r for r in rows if int(r.get("event_index", 1)) == 1]
        if first and len(first) != len(rows):
            out["first_event_only"] = _summarize_rows(first, nest=True)
    return out


def _verdict(metrics: dict) -> dict:
    i500 = metrics.get("I_500") or {}
    i100 = metrics.get("I_100") or {}
    i250 = metrics.get("I_250") or {}
    p500 = i500.get("P_I_lt_0", float("nan"))
    med500 = i500.get("median_cm", float("nan"))
    p100 = i100.get("P_I_lt_0", float("nan"))
    p250 = i250.get("P_I_lt_0", float("nan"))
    by = metrics.get("by_terrain") or {}
    slope_p = (by.get("slope") or {}).get("I_500", {}).get("P_I_lt_0", float("nan"))
    steps_p = (by.get("steps") or {}).get("I_500", {}).get("P_I_lt_0", float("nan"))
    rough_ok = True
    for ter in ("slope", "steps"):
        pv = (by.get(ter) or {}).get("I_500", {}).get("P_I_lt_0", float("nan"))
        if np.isfinite(pv):
            rough_ok = rough_ok and pv >= 0.70
        else:
            rough_ok = False
    go = bool(np.isfinite(p500) and p500 >= 0.70 and np.isfinite(med500) and med500 < 0.0 and rough_ok)
    tc = bool(np.isfinite(p500) and p500 < 0.55)
    tb = bool(np.isfinite(p100) and p100 >= 0.70 and np.isfinite(p500) and 0.55 <= p500 < 0.70)
    ta = bool(go and np.isfinite(p100) and p100 < 0.60 and p500 >= 0.80)
    if tc:
        case, next_step = "T-C", "STOP: live mismatch; do not CL2 / PPO / 6V"
    elif go:
        case = "T-A" if ta else "T-GO"
        next_step = "GO CL2"
    elif tb:
        case, next_step = "T-B", "GO CL2 (burst helps early; 500 ms fades — recurrent may be needed)"
    else:
        case, next_step = "HOLD", "HOLD: inspect I_100/I_250/I_500 before CL2"
    return {
        "GO": go,
        "case": case,
        "next": next_step,
        "P_I500_lt_0": p500,
        "median_I500_cm": med500,
        "P_I100_lt_0": p100,
        "P_I250_lt_0": p250,
        "slope_P_I500": slope_p,
        "steps_P_I500": steps_p,
        "rough_ge_70": rough_ok,
        "offline_quality": OFFLINE_P["quality"],
        "offline_exact": OFFLINE_P["exact"],
    }
...
def _eval_snaps(
    ...
    for start in range(0, len(snaps), n_envs):
        ...
        n_valid = len(batch)
        print(f"[6s-t] probe batch {start}:{start + n_valid} / {len(snaps)}", flush=True)
        ...
        for i, s in enumerate(batch):
            ...
            row = {
                ...
                "E0_cm": e0 * 100.0,
                "R_E": float(s["R_E"]),
                "event_index": int(s.get("event_index", 1)),
            }
        ...
        print(
            f"[6s-t] running n={len(rows)} P(I_500<0)={p500:.3f} "
...
def _tag_event_index(snaps: list[dict]) -> None:
    """1-based index of this trigger within (terrain, seed, clip)."""
    counts: dict[tuple, int] = {}
    for s in snaps:
        key = (s["terrain"], int(s["seed"]), s["clip"])
        counts[key] = counts.get(key, 0) + 1
        s["event_index"] = counts[key]
...
def _run_terrain(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str, out_root: Path) -> dict:
    ...
    if done_p.exists():
        print(f"[6s-t] skip existing {done_p}", flush=True)
    ...
    for seed in seeds:
        if cap > 0 and len(all_snaps) >= cap:
            break
        print(f"[6s-t] ===== collect {terrain} seed={seed} =====", flush=True)
        torch.manual_seed(seed)
        np.random.seed(seed)
        _use, paths = _pin_clips(env, mask_name="torso")
        remain = (cap - len(all_snaps)) if cap > 0 else 0
    ...
    _tag_event_index(all_snaps)
    print(f"[6s-t] {terrain} n_triggers={len(all_snaps)} dt=0", flush=True)
    rows = _eval_snaps(
    ...
    payload = {
        ...
        "metrics": metrics,
        "rows": rows,
        "offline_ref": OFFLINE_P,
        "note": "I_h = E^R_{t+h} - E^P_{t+h}. Absolute ΔE@100ms is diagnostic only.",
    }
    ...
    i500 = metrics.get("I_500") or {}
    print(
        f"[6s-t] {terrain} n={len(rows)} "
        f"P(I100<0)={(metrics.get('I_100') or {}).get('P_I_lt_0')} "
        f"P(I250<0)={(metrics.get('I_250') or {}).get('P_I_lt_0')} "
        f"P(I500<0)={i500.get('P_I_lt_0')} median500={i500.get('median_cm')}cm",
        flush=True,
    )
    try:
        env.close()
...
def _write_pooled(out_root: Path, resume_path: str) -> dict | None:
    ...
    print(
        f"[6s-t] POOLED n={len(rows)} case={gates['case']} GO={gates['GO']} "
        f"P(I500<0)={gates['P_I500_lt_0']} med={gates['median_I500_cm']}cm "
        f"next={gates['next']}",
        flush=True,
    )
    return pooled
...
def main(env_cfg, agent_cfg):
    ...
    print(f"[6s-t] ckpt {resume_path}", flush=True)
    print("[6s-t] live-trigger paired I_500. NO PPO. NO CL2.", flush=True)
