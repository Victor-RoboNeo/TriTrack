#!/usr/bin/env python3
"""Step 6S-CL — Supervised closed-loop intent recovery. No PPO. No 6V.

Frozen parent: Mapper-B + model_50000. Frozen 6S MLP. Loco torso. Step-3 gate.

Binary authority (no continuous alpha):
    active=0 → 0°
    active=1 → fixed 5°

100 ms chunk: predict once, hold raw direction for 5 steps, re-project onto
each current z_nom. Do not re-query the network inside the burst.

CL1: one 5°/100 ms burst per recovery event, then Parent until release.
CL2: after every burst, re-evaluate RE; release if <0.6 persist-3, else re-predict.
"""
...
PARENT_SR5 = {"plane": 0.535, "light_rough": 0.549, "slope": 0.348, "steps": 0.316}
PARENT_SR_RECOVER = {"plane": 0.824, "light_rough": 0.768, "slope": 0.412, "steps": 0.375, "all": 0.573}
...
parser = argparse.ArgumentParser(description="P2-R Step 6S supervised closed-loop recovery.")
...
parser.add_argument("--out", type=str, default="results/p2r_step6s_cl")
...
parser.add_argument("--mode", type=str, default="cl1", choices=("cl1", "cl2"))
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="0 = unlimited (CL2). 2 = force Parent after 2 bursts/event (6S-R2).",
)
...
def _apply_terrain(env_cfg, terrain: str) -> None:
    if terrain == "plane":
        ...
        print("[6s-cl] terrain=plane", flush=True)
        return
    ...
    print(f"[6s-cl] terrain={terrain} 1x1 generator", flush=True)
    ...
    with torch.inference_mode():
        ...
        if mask_name and hasattr(cmd, "set_eval_fixed_mask_mode_idx"):
            ...
            if mask_name in names:
                ...
                print(f"[6s-cl] pin mask {mask_name!r} idx={names.index(mask_name)}", flush=True)
        ...
        try:
            ...
            print(f"[6s-cl] sim.forward skipped: {exc}", flush=True)
    ...
    *,
    ...
    for t in range(steps):
        ...
        if (t + 1) % 50 == 0:
            ...
            print(
                f"[6s-cl] {mode} {terrain} s{seed} step {t+1}/{steps} "
...
def _run_terrain(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str, out_root: Path) -> dict:
    ...
    if done_p.exists():
        print(f"[6s-cl] skip existing {done_p}", flush=True)
    ...
    all_eps: list[dict] = []
    for seed in seeds:
        print(f"[6s-cl] ===== mode={mode} terrain={terrain} seed={seed} =====", flush=True)
        ...
        print(f"[6s-cl] seed={seed} events={n_trig} bursts={n_b} eps={len(ep_rows)}", flush=True)
    ...
    print(
        f"[6s-cl] {terrain} SR@5={sr5:.3f} (parent {parent_sr5:.3f}, Δ={payload['delta_sr_5cm_pp']:.1f}pp) "
...
def _write_pooled(out_root: Path, resume_path: str) -> dict | None:
    ...
    d_flat = by_terrain.get("plane", {}).get("delta_sr_5cm_pp", float("nan"))
    d_light = by_terrain.get("light_rough", {}).get("delta_sr_5cm_pp", float("nan"))
    slope_rec = by_terrain.get("slope", {}).get("SR_recover", float("nan"))
    steps_rec = by_terrain.get("steps", {}).get("SR_recover", float("nan"))
    burst2 = recov_all["by_burst_index"].get("2", {})
    mechanism_go = bool(np.isfinite(med) and med < 0.0)
    stop_a = bool(np.isfinite(med) and med > 0.0 and (not np.isfinite(u1) or u1 < 0.55))
    stop_b = bool(
        mechanism_go
        and burst2.get("n", 0) > 0
        and np.isfinite(burst2.get("median_dE_cm", float("nan")))
        and burst2["median_dE_cm"] > 0.0
    )
    stop_c = bool(
        (np.isfinite(d_flat) and d_flat < -5.0) or (np.isfinite(d_light) and d_light < -5.0)
    )
    slope_rec_go = bool(np.isfinite(slope_rec) and slope_rec > PARENT_SR_RECOVER["slope"])
    steps_rec_go = bool(np.isfinite(steps_rec) and steps_rec > PARENT_SR_RECOVER["steps"])
    pooled = {
        "step": f"6S-{args_cli.mode}",
        "no_ppo": True,
        "ckpt": resume_path,
        "step6s_ckpt": args_cli.step6s_ckpt,
        "mode": args_cli.mode,
        ...
        "gates": {
            "mechanism_go_median_dE1_lt_0": mechanism_go,
            "slope_SRrecover_gt_parent": slope_rec_go,
            "steps_SRrecover_gt_parent": steps_rec_go,
            "STOP_A_first_burst_wrong": stop_a,
            "STOP_B_burst1_ok_later_fail": stop_b,
            "STOP_C_flat_light_harm": stop_c,
            "U1_live": u1,
            "U1_offline_quality": OFFLINE_U5_QUALITY,
            "median_dE1_cm": med,
        },
    }
    (out_root / "pooled.json").write_text(json.dumps(_sanitize(pooled), indent=2), encoding="utf-8")
    print(
        f"[6s-cl] POOLED mode={args_cli.mode} n_ep={len(all_eps)} n_ev={recov_all['n_events']} "
        f"U1={u1} medianΔE1={med}cm mech_GO={mechanism_go} "
        f"STOP_A={stop_a} STOP_B={stop_b} STOP_C={stop_c}",
        flush=True,
    )
    return pooled
...
def main(env_cfg, agent_cfg):
    ...
    print(f"[6s-cl] ckpt {resume_path}", flush=True)
    print(
        f"[6s-cl] mode={args_cli.mode} theta={args_cli.theta_deg} "
        f"burst={args_cli.burst_steps} max_bursts={args_cli.max_bursts} NO PPO",
        flush=True,
    )
