#!/usr/bin/env python3
"""Phase T1.1 stress generator validation + Reach human-intent API repair.

Frozen Parent. No training / PPO / oracle / R-M3 update.
R-M3 shadow only (z_exec = z_nom).
"""
...
parser = argparse.ArgumentParser(description="P2-R Phase T1 failure-stress frozen eval. No PPO.")
...
def _stress_meta() -> dict:
    ...
    return {
        ...
        "push_step": push_step,
        "intent_scale": float(args_cli.intent_scale),
        "intent_mode": str(args_cli.intent_mode),
...
def _scale_human_wrist_intent(env, mask_name: str) -> dict:
    """Scale human wrist trajectory relative to torso in MultiMotionLoader BEFORE Mapper-B.

    Source of truth: cmd.motion_dir_loader.body_pos_w [T_all, n_bodies, 3]
    (PartialMaskedMultiMotionCommand has no .motion; T1 cmd.motion was the bug.)
    Mapper-B reads the same tensor via causal_future.gather_ref_world → loader.gather.
    Orientation (body_quat_w) is left unchanged.
    """
    scale = float(args_cli.intent_scale)
    mode = str(args_cli.intent_mode)
    info = {
        "applied": False,
        "api": "motion_dir_loader.body_pos_w",
        "before_mapper_b": True,
        "scale": scale,
        "mode": mode,
        "bodies": [],
        "median_torso_wrist_before_m": None,
        "median_torso_wrist_after_m": None,
        "impossible": False,
        "old_bug": "T1 accessed cmd.motion which does not exist on PartialMaskedMultiMotionCommand",
    }
    ...
    if loader is None or not hasattr(loader, "body_pos_w"):
        raise RuntimeError("Reach intent source missing: command.motion_dir_loader.body_pos_w")
    if getattr(loader, "_t11_intent_scaled", False):
        return getattr(loader, "_t11_intent_info", info)
    ...
    targets = [b for b in _ws_body_names(mask_name) if b in body_names]
    bp = loader.body_pos_w
    med0 = None
    if targets:
        wi0 = body_names.index(targets[0])
        med0 = float(torch.linalg.norm(bp[:, wi0] - bp[:, ti], dim=-1).median().item())
    if abs(scale - 1.0) < 1e-9:
        info.update({"applied": False, "bodies": targets, "median_torso_wrist_before_m": med0,
                     "median_torso_wrist_after_m": med0})
        loader._t11_intent_scaled = True
        loader._t11_intent_info = info
        print(f"[t11] intent scale=1.0 (no-op) {info}", flush=True)
        return info
    for name in targets:
        wi = body_names.index(name)
        rel = bp[:, wi] - bp[:, ti]
        rel2 = rel.clone()
        if mode == "radial":
            rel2 = scale * rel
            ...
            rel2[:, 1] = -scale * rel[:, 1]
        bp[:, wi] = bp[:, ti] + rel2
    loader.body_pos_w = bp
    med1 = None
    if targets:
        wi0 = body_names.index(targets[0])
        med1 = float(torch.linalg.norm(bp[:, wi0] - bp[:, ti], dim=-1).median().item())
    info.update({
        "applied": True,
        "bodies": targets,
        "median_torso_wrist_before_m": med0,
        "median_torso_wrist_after_m": med1,
        "impossible": bool(med1 is not None and med1 > 0.85),
    })
    loader._t11_intent_scaled = True
    loader._t11_intent_info = info
    print(f"[t11] human-intent scale {info}", flush=True)
    if info["impossible"]:
        print(f"[t11] WARN median torso-wrist {med1:.3f}m may be kinematically impossible", flush=True)
    return info
...
def _make_env(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str):
    ...
    raw._t1_ws_info = _scale_human_wrist_intent(env, mask_name)
...
def _rollout_cl(
    ...
    *,
    ...
    first_trig = [-1] * use
    body_series = {
        (seed, e): {"torso": [], "lw": [], "rw": [], "S_raw": [], "R_E": [], "R_S": [], "R": []}
        for e in range(use)
    }
    meta = _stress_meta()
    push_step = int(meta["push_step"])
    push_dv = float(meta["push_dv_mps"])
    push_axis = str(meta["push_axis"])
    pushed = False
    push_info = {"applied": False, "command_dv": push_dv, "measured_dv_mean": 0.0}
    first_re = [-1] * use
    first_rs = [-1] * use
    phys = {
        (seed, e): {
    ...
    for t in range(steps):
        ...
        joints = _decode(policy, z_exec, proprio)
        env.step(joints)
        if intent_speed > 1.0 + 1e-6:
            speed_acc += intent_speed - 1.0
            extra = int(speed_acc)
            speed_acc -= extra
            if extra > 0:
                mx = cmd.motion_lengths_minus_one[cmd.env_motion_indices]
                cmd.time_steps = torch.minimum(cmd.time_steps + extra, mx)
        ...
        if (t + 1) % 50 == 0:
            ...
            print(
                f"[t1] {rec_kind} {task_source} {terrain} s{seed} step {t+1}/{steps} "
...
def _run_terrain(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str, out_root: Path) -> dict:
    ...
    payload = {
        ...
        "stress": _stress_meta(),
        "workspace": getattr(env.unwrapped, "_t1_ws_info", {}),
    }
    ...
    print(
        f"[t11] {task}/{rec_kind}/{terrain} SR_task={mon['sr_task']:.3f} "
        f"fail={mon['fail_frac']:.3f} phys={mon.get('physics')}",
        flush=True,
    )
    try:
        env.close()
