#!/usr/bin/env python3
"""Phase R-M Stoop oracle — task-preserving local controllability. No PPO.

Canonical parent: Mapper-B + model_50000. Frozen Loco 6S is evaluated, never trained.
Stoop clips, mask vr, gate R = max(R_E, R_S). Terrain is an analysis label only.

For each S-first / E-first trigger: clone sim; 15-D tangent ±1° Probe-B 100 ms;
parent continuation to 0.5 s; d*_stoop; sweep 1/2.5/5/7.5/10°; compare Loco 6S
predicted direction on the same clone.

I_h = E^R_{t+0.5} - E^P_{t+0.5}. Want I*<0. Probe-A / +100 ms consistency off by default.
"""
...
parser = argparse.ArgumentParser(description="P2-R Stoop task-preserving local oracle. No PPO.")
...
parser.add_argument("--out", type=str, default="results/p2r_rm_stoop_oracle")
...
parser.add_argument("--max_states", type=int, default=65)
parser.add_argument("--max_states_s", type=int, default=40, help="S-only cap per terrain.")
parser.add_argument("--max_states_e", type=int, default=15, help="E-only cap per terrain.")
parser.add_argument("--max_states_both", type=int, default=10, help="both-channel cap per terrain.")
...
parser.add_argument(
    ...
    type=str,
    default=STEP6S_CKPT,
    help="Frozen Loco 6S. Compared on the same clone after d*; never skips the FD oracle.",
)
parser.add_argument(
    "--step6s_split",
    type=str,
...
parser.add_argument(
    "--no_loco_cmp",
    action="store_true",
    help="Skip frozen 6S comparison (dataset collection only).",
)
...
if bool(getattr(args_cli, "no_loco_cmp", False)):
    args_cli.step6s_ckpt = ""
_ckpt6 = str(getattr(args_cli, "step6s_ckpt", "") or "")
if _ckpt6.lower() in ("none", "null", "off"):
    args_cli.step6s_ckpt = ""
...
def _collect_triggers(
    ...
    *,
    ...
    n_envs = int(env.unwrapped.num_envs)
    gate = RecoveryRiskGate(s_enabled=True)
    gate.reset()
...
def _make_env(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str):
    ...
    spec, probs = eval_single_mode_spec("vr")
...
def _run_terrain(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str, out_dir: Path) -> list[dict]:
    ...
    done_p = cell / "states.json"
    if done_p.exists():
        print(f"[rm-stoop-ora] skip existing {done_p}", flush=True)
    ...
    all_snaps: list[dict] = []
    for seed in seeds:
        print(f"[rm-stoop-ora] collect terrain={terrain} seed={seed}", flush=True)
    ...
    all_snaps = _cap_by_channel(all_snaps)
    print(f"[rm-stoop-ora] {terrain}: probing n={len(all_snaps)} vis={vis_flags}", flush=True)
    rows = _probe_states(
        ...
        step6s_ckpt=str(getattr(args_cli, "step6s_ckpt", "") or ""),
    )
    cell.mkdir(parents=True, exist_ok=True)
    (cell / "states.json").write_text(json.dumps(rows))
    env.close()
    print(f"[rm-stoop-ora] wrote {cell / 'states.json'} n={len(rows)}", flush=True)
    return rows


def _write_summary(out_dir: Path, rows: list[dict], resume_path: str) -> dict:
    summary = {
        "step": "rm-stoop-oracle",
        "no_ppo": True,
        "no_train_6s": True,
        "ckpt": resume_path,
        ...
        "scatter": [
            {
                ...
                "I_cm": r["I_oracle_cm"],
                "I_loco_5deg_cm": r.get("I_loco_5deg_cm"),
                "cos_loco_dstar": r.get("cos_loco_dstar"),
                "best_angle_deg": r.get("best_angle_deg"),
                "case": r["case"],
                "R_E": r["R_E"],
                "R_S": r.get("R_S"),
            }
    ...
    def _f(x, k="P_I_lt_0"):
        v = (x or {}).get(k, float("nan"))
        return float(v) if v == v else float("nan")

    print(
        f"[rm-stoop-ora] summary n={m.get('n', 0)} "
        f"P(I*<0)={m.get('P_I_lt_0', float('nan')):.3f} "
        f"P(I*<-1cm)={m.get('P_I_lt_1cm', float('nan')):.3f} "
        f"S-first n={sf.get('n', 0)} P(I*<0)={_f(sf):.3f} "
        f"medI*={sf.get('median_oracle_advantage_cm', float('nan'))} "
        f"E-first n={ef.get('n', 0)} P(I*<0)={_f(ef):.3f} "
        f"medI*={ef.get('median_oracle_advantage_cm', float('nan'))} "
        f"S cos={sf.get('median_cos_loco_dstar', float('nan'))} "
        f"S I_loco={((sf.get('I_loco_5deg_cm') or {}).get('p50', float('nan')))}",
        flush=True,
    )
    print(f"[rm-stoop-ora] wrote {out_dir / 'summary.json'}", flush=True)
    return summary
...
def main(env_cfg, agent_cfg):
    ...
    print(f"[rm-stoop-ora] ckpt {resume_path}", flush=True)
    ...
    print(
        f"[rm-stoop-ora] motion={motion_dir} mask={args_cli.mask_modes} "
        f"6S={getattr(args_cli, 'step6s_ckpt', '')} skipA={args_cli.skip_probe_a} "
        f"skipCons={args_cli.skip_consistency}",
        flush=True,
    )
    for terrain in terrains:
        print(f"[rm-stoop-ora] ===== {terrain} =====", flush=True)
    ...
    _write_summary(out_dir, _load_all_rows(out_dir), resume_path)
    print("[rm-stoop-ora] done", flush=True)
