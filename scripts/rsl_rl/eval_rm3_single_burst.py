#!/usr/bin/env python3
"""Phase R-M3 — Shared intent-conditioned single-burst closed-loop. Frozen eval.

ONE burst per event (100 ms @ 50 Hz). Hold raw direction; re-project onto
current z_nom every step; return to Parent. No Max-2, no continuation, no PPO.
"""
...
FAIL_ANCHOR_Z = 0.25
FAIL_ANCHOR_ORI = 0.8
REC_KINDS = ("none", "6s", "shared5", "shared_adapt")
...
parser = argparse.ArgumentParser(description="P2-R Phase R-M3 single-burst frozen eval. No PPO.")
...
parser.add_argument("--task_source", type=str, default="loco", choices=("loco", "stoop"))
...
parser.add_argument("--capture_snaps", action="store_true", default=False)
parser.add_argument("--run_paired", action="store_true", default=False)
parser.add_argument("--max_snaps", type=int, default=30)
parser.add_argument("--paired_out", type=str, default="")
...
if str(args_cli.task_source) == "stoop" and not bool(getattr(args_cli, "no_s_enabled", False)):
...
def _apply_terrain(env_cfg, terrain: str) -> None:
    if terrain == "plane":
        ...
        print("[rm3] terrain=plane", flush=True)
        return
    ...
    print(f"[rm3] terrain={terrain} 1x1 generator", flush=True)
    ...
    with torch.inference_mode():
        ...
        if mask_name and hasattr(cmd, "set_eval_fixed_mask_mode_idx"):
            ...
            if mask_name in names:
                ...
                print(f"[rm3] pin mask {mask_name!r} idx={names.index(mask_name)}", flush=True)
        ...
        try:
            ...
            print(f"[rm3] sim.forward skipped: {exc}", flush=True)
...
def _paired_on_snaps(
    ...
    for start in range(0, len(snaps), n_envs):
        ...
        n_valid = len(batch)
        print(f"[rm3] paired batch {start}:{start + n_valid}/{len(snaps)} {task}/{terrain}", flush=True)
        e_p = _rollout_probe(
    ...
    print(
        f"[rm3] paired {task}/{terrain} n={len(rows)} "
    ...
    for t in range(steps):
        ...
        for i in range(use):
            ...
            if bool(rising[i]) and t >= WARMUP:
                ...
                if capture_snaps and _want_snap(snaps, ch, task_source, max_snaps):
                    if hist is None:
        ...
        if (t + 1) % 50 == 0:
            ...
            print(
                f"[rm3] {rec_kind} {task_source} {terrain} s{seed} step {t+1}/{steps} "
...
def _run_terrain(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str, out_root: Path) -> dict:
    ...
    paired_root = Path(args_cli.paired_out) if args_cli.paired_out else (out_root.parent.parent / "paired_clone")
    task = str(args_cli.task_source)
    rec_kind = str(args_cli.rec_kind)
    paired_cell = paired_root / task / terrain
    paired_json = paired_cell / "i_live.json"
    skip_live = done_p.exists()
    skip_paired = paired_json.exists() or not bool(args_cli.run_paired)
    if skip_live and skip_paired:
        print(f"[rm3] skip existing {done_p}", flush=True)
        return json.loads(done_p.read_text())

    env, runner, policy, injector = _make_env(env_cfg, agent_cfg, resume_path, motion_dir, terrain)
    sixs, mu6, sd6 = _load_6s_predictor(args_cli.step6s_ckpt, env.device)
    ...
    if not skip_live:
        for seed in seeds:
            print(
                f"[rm3] ===== kind={rec_kind} task={task} s={int(s_enabled)} "
            ...
            events, ep_rows = _rollout_cl(
                ...
                mask_name=mask_name,
                task_source=task,
                capture_snaps=bool(args_cli.capture_snaps),
                max_snaps=int(args_cli.max_snaps),
                snaps=snaps,
            )
            all_events.extend(events)
            all_eps.extend(ep_rows)
            print(
                f"[rm3] seed={seed} events={len(events)} bursts={sum(e['n_bursts'] for e in events)} "
                f"snaps={len(snaps)}",
                flush=True,
            )
        ...
        complete = float(np.mean([r["task_completion"] for r in all_eps])) if all_eps else float("nan")
        payload = {
            "step": "R-M3",
            "no_ppo": True,
            "single_burst": True,
            ...
            "events_per_episode": (len(all_events) / len(all_eps)) if all_eps else float("nan"),
            "trigger_count": len(all_events),
            "n_snaps_captured": len(snaps),
            "recovery": recov,
            "episodes": all_eps,
            "events": all_events,
        }
        ...
        print(
            f"[rm3] {task}/{rec_kind}/{terrain} SR@5={sr5:.3f} fail={fail_frac:.3f} "
            f"complete={complete:.3f} n_ev={recov['n_events']} snaps={len(snaps)}",
            flush=True,
        )
        if snaps:
            paired_cell.mkdir(parents=True, exist_ok=True)
            torch.save(snaps, paired_cell / "snaps.pt")
            meta_snaps = [
                {
                    "clip": s.get("clip"),
                    "seed": s.get("seed"),
                    "t0": s.get("t0"),
                    "trigger_channel": s.get("trigger_channel"),
                    "E0": s.get("E0"),
                    "terrain": s.get("terrain"),
                    "task_source": s.get("task_source"),
                }
                for s in snaps
            ]
            (paired_cell / "snaps_meta.json").write_text(
                json.dumps(_sanitize(meta_snaps), indent=2), encoding="utf-8"
            )
    else:
        payload = json.loads(done_p.read_text())
        print(f"[rm3] live skip {done_p}", flush=True)

    if bool(args_cli.run_paired) and not skip_paired:
        if not snaps:
            snap_pt = paired_cell / "snaps.pt"
            if snap_pt.exists():
                snaps = torch.load(snap_pt, map_location="cpu", weights_only=False)
        if snaps:
            paired_cell.mkdir(parents=True, exist_ok=True)
            torch.save(snaps, paired_cell / "snaps.pt")
            paired = _paired_on_snaps(
                env, policy, runner, injector, snaps, vis_i, vis_flags,
                sixs, mu6, sd6, adapter, mua, sda, bins, task, terrain,
            )
            (paired_cell / "i_live.json").write_text(json.dumps(_sanitize(paired), indent=2), encoding="utf-8")
            meta = {k: v for k, v in paired.items() if k != "rows"}
            (paired_cell / "summary.json").write_text(json.dumps(_sanitize(meta), indent=2), encoding="utf-8")
        else:
            print(f"[rm3] paired requested but no snaps for {task}/{terrain}", flush=True)
...
def _write_pooled(out_root: Path, resume_path: str) -> dict | None:
    ...
    for ter, cell in cells.items():
        ...
        by_terrain[ter] = {
            ...
            "events_per_episode": cell.get("events_per_episode"),
            "fail_after_trigger": recov.get("fail_after_trigger"),
            "trigger_channel": recov.get("trigger_channel"),
            "S_first": recov.get("S_first"),
            "E_first": recov.get("E_first"),
            "AUC_E": recov.get("AUC_E"),
            "theta": recov.get("theta"),
    ...
    pooled = {
        ...
        "sr_5cm_all": sr5_all,
        "fail_frac_all": float(np.mean([r["fail"] for r in all_eps])) if all_eps else float("nan"),
        "task_completion_all": float(np.mean([r["task_completion"] for r in all_eps])) if all_eps else float("nan"),
        "recovery_all": recov_all,
        "by_terrain": by_terrain,
        "note": "Frozen single-burst intent-conditioned recovery. No PPO. No Max-2.",
    }
    (out_root / "pooled.json").write_text(json.dumps(_sanitize(pooled), indent=2), encoding="utf-8")
    print(
        f"[rm3] POOLED {args_cli.task_source}/{args_cli.rec_kind} n_ep={len(all_eps)} "
        f"n_ev={recov_all['n_events']} SR@5={sr5_all:.3f}",
        flush=True,
    )
    return pooled
...
def main(env_cfg, agent_cfg):
    ...
    print(f"[rm3] ckpt {resume_path}", flush=True)
    print(
        f"[rm3] task={args_cli.task_source} kind={args_cli.rec_kind} "
        f"theta={args_cli.theta_deg} burst={args_cli.burst_steps} max_bursts={args_cli.max_bursts} "
        f"s_enabled={args_cli.s_enabled} mask={args_cli.mask_modes} capture={args_cli.capture_snaps} "
        f"paired={args_cli.run_paired} NO PPO",
        flush=True,
    )
