#!/usr/bin/env python3
"""Phase T1 — Failure-stress Human Intent Transfer. Frozen Parent eval. No training.

Parent: Mapper-B + g_phi,50000. Task SR = official Locomani termination success
(fall / anchor_z / anchor_ori). SR@5 is secondary only.

Stress families (no new object, no depth injection):
  A payload  — extra mass split on L/R wrist_yaw_link (no carry rigid object exists)
  B com      — lateral/forward wrist COM offset, typically on top of a fixed payload
  C workspace— clip-frame offset of the Reach right-wrist target (feasible extension)
  D push     — single robot-frame lateral Δv via write_root_velocity_to_sim

R-M3: Loco/Stoop may be active comparator. Reach/Carry always shadow (z_exec=z_nom).
No PPO, no oracle, no adapter update, no Reach/Carry active recovery.
"""
...
from isaaclab.utils.math import quat_apply, quat_rotate_inverse  # noqa: E402
...
def _apply_workspace_offset(env, mask_name: str) -> dict:
    fwd, lat, zz = float(args_cli.ws_fwd), float(args_cli.ws_lat), float(args_cli.ws_z)
    info = {"applied": False, "bodies": [], "fwd": fwd, "lat": lat, "z": zz, "median_torso_wrist_m": None}
    if all(abs(v) < 1e-12 for v in (fwd, lat, zz)):
        return info
    ...
    bp = cmd.motion.body_pos_w
    if getattr(cmd.motion, "_t1_ws_applied", False):
        return getattr(cmd.motion, "_t1_ws_info", info)
    offset = torch.tensor([fwd, lat, zz], dtype=bp.dtype)
    for name in targets:
        idx = body_names.index(name)
        bp[:, idx, :] = bp[:, idx, :] + offset.to(device=bp.device)
    cmd.motion.body_pos_w = bp
    cmd.motion._t1_ws_applied = True
    med = None
    if "torso_link" in body_names and targets:
        ti = body_names.index("torso_link")
        wi = body_names.index(targets[0])
        dist = torch.linalg.norm(bp[:, wi] - bp[:, ti], dim=-1)
        med = float(dist.median().item())
    info.update({"applied": True, "bodies": targets, "median_torso_wrist_m": med})
    cmd.motion._t1_ws_info = info
    print(f"[t1] workspace offset {info}", flush=True)
    if med is not None and med > 0.85:
        print(f"[t1] WARN workspace median torso-wrist {med:.3f}m may be kinematically hard", flush=True)
    return info


def _apply_push(asset, n_use: int, dv: float, axis: str) -> None:
    if abs(dv) < 1e-9:
        return
    ...
    print(f"[t1] push applied axis={axis} dv={dv:.3f} n={n_use}", flush=True)
...
def _make_env(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str):
    ...
    raw._t1_ws_info = _apply_workspace_offset(env, mask_name)
...
def _rollout_cl(
    ...
    *,
    ...
    for t in range(steps):
        if (not pushed) and abs(push_dv) > 1e-9 and t == push_step:
            _apply_push(asset, use, push_dv, push_axis)
            pushed = True
        obs, _ = env.get_observations()
        obs = injector.patch_policy_obs(obs, env, "mapper")
...
def _run_terrain(env_cfg, agent_cfg, resume_path: str, motion_dir: str, terrain: str, out_root: Path) -> dict:
    ...
    print(
        f"[t1] {task}/{rec_kind}/{terrain} SR_task={mon['sr_task']:.3f} SR@5={mon['sr_5cm']:.3f} "
