#!/usr/bin/env python3
"""P3-B2: freeze B1, sweep learned λ. Frozen Stage-2. Oracle λ=1."""
...
OUT_DEFAULT = "results/p3b2_conservative_projector"
...
parser = argparse.ArgumentParser(description="P3-B2 B1 lambda calibration. Frozen Stage-2. Oracle λ=1.")
...
parser.add_argument("--stage", type=str, default="calib", choices=("calib", "rollout", "shield", "b3", "b4b"))
parser.add_argument("--ucb_beta", type=float, default=1.0)
parser.add_argument("--ensemble_ckpts", type=str, default="")
parser.add_argument("--b3_out", type=str, default="results/p3b3_tail_risk")
parser.add_argument("--ifs_root", type=str, default=IFS_ROOT)
parser.add_argument("--p3a_root", type=str, default="results/p3_intent_projected_adaptation")
parser.add_argument("--b1_ckpt", type=str, default="")
parser.add_argument("--b21_ckpt", type=str, default="")
parser.add_argument("--lambdas", type=str, default="1.0,1.25,1.5,2.0,3.0,5.0")
parser.add_argument("--selected_json", type=str, default="")
parser.add_argument("--models", type=str, default="B1")
parser.add_argument("--n_pert_per_nom", type=int, default=1)
parser.add_argument("--max_nominal", type=int, default=600)
parser.add_argument("--max_offman", type=int, default=600)
...
def geodesic_z(z: torch.Tensor, v: torch.Tensor, eps) -> torch.Tensor:
    z = F.normalize(z, dim=-1, eps=1e-8)
    v = project_tangent(v, z)
    if torch.is_tensor(eps) and eps.numel() > 1:
        e = eps.reshape(-1, 1).to(device=z.device, dtype=z.dtype)
        return F.normalize(torch.cos(e) * z + torch.sin(e) * v, dim=-1, eps=1e-8)
...
@torch.inference_mode()
def _extract_x(env, policy, runner, obs, vis_i, vis_flags, ankle_i, n: int) -> tuple[torch.Tensor, torch.Tensor]:
    cmd = env.unwrapped.command_manager.get_term("motion")
    asset = env.unwrapped.scene["robot"]
    origins = env.unwrapped.scene.env_origins
    am = env.unwrapped.action_manager
    z, proprio, _enc = _z_and_proprio(policy, runner, obs)
    I_parts, e_parts, m_parts = [], [], []
    for k, flag in enumerate(vis_flags):
        m_parts.append(torch.full((n, 1), 1.0 if flag else 0.0, device=z.device, dtype=z.dtype))
        pref = cmd.body_pos_w[:n, vis_i[k]].clone()
        prob = cmd.robot_body_pos_w[:n, vis_i[k]].clone()
        pref[:, 0] -= origins[:n, 0]
        pref[:, 1] -= origins[:n, 1]
        prob[:, 0] -= origins[:n, 0]
        prob[:, 1] -= origins[:n, 1]
        I_parts.append(pref)
        e_parts.append(prob - pref)
    I = torch.cat(I_parts, dim=-1)
    eI = torch.cat(e_parts, dim=-1)
    m = torch.cat(m_parts, dim=-1)
    g_w = torch.tensor([0.0, 0.0, -1.0], device=z.device, dtype=z.dtype).unsqueeze(0).expand(n, -1)
    g = quat_rotate_inverse(asset.data.root_quat_w[:n], g_w)
    c2 = _contact2(env, ankle_i, n)
    cbin = (c2 > CONTACT_N).float()
    x = torch.cat(
        [
            z[:n],
            I,
            m,
            eI,
            asset.data.joint_pos[:n],
            asset.data.joint_vel[:n],
            g,
...
def _perturb_snap(snap: dict, rng: np.random.RandomState) -> dict:
    s = dict(snap)
    root = snap["root_local"].clone()
    jp = snap["joint_pos"].clone()
    kind = int(rng.randint(3))
    if kind in (0, 1):
        roll = float((rng.rand() - 0.5) * math.radians(12.0))
        pitch = float((rng.rand() - 0.5) * math.radians(12.0))
        cr, sr = math.cos(roll / 2.0), math.sin(roll / 2.0)
        cp, sp = math.cos(pitch / 2.0), math.sin(pitch / 2.0)
        dw, dx, dy, dz = cr * cp, sr * cp, cr * sp, sr * sp
        qw, qx, qy, qz = [float(v) for v in root[3:7]]
        root = root.clone()
        root[3] = qw * dw - qx * dx - qy * dy - qz * dz
        root[4] = qw * dx + qx * dw + qy * dz - qz * dy
        root[5] = qw * dy - qx * dz + qy * dw + qz * dx
        root[6] = qw * dz + qx * dy - qy * dx + qz * dw
        root[7:10] = root[7:10] + torch.as_tensor(rng.randn(3) * 0.25, dtype=root.dtype)
        root[10:13] = root[10:13] + torch.as_tensor(rng.randn(3) * 0.35, dtype=root.dtype)
    if kind in (1, 2):
        root = root.clone()
        root[2] = root[2] + float((rng.rand() - 0.5) * 0.06)
        root[7:10] = root[7:10] + torch.as_tensor(rng.randn(3) * 0.20, dtype=root.dtype)
        jp = jp + torch.as_tensor(rng.randn(int(jp.numel())) * 0.03, dtype=jp.dtype).clamp(-0.08, 0.08)
    s["root_local"] = root
    s["joint_pos"] = jp
    s["window"] = "perturbed"
    s["parent_episode_id"] = str(snap.get("parent_episode_id", snap["episode_id"]))
    s["episode_id"] = str(s["parent_episode_id"]) + f"|p{int(rng.randint(1e9))}"
    s["state_kind"] = "perturbed"
    return s
...
def _collect_snaps(env, policy, runner, injector, steps, seed, terrain, vis_i, vis_flags, mask_name, layout):
    ...
    for t in range(steps):
        ...
        if t >= WARMUP:
            ...
            x_t, _z_live = _extract_x(env, policy, runner, obs, vis_i, vis_flags, ankle_i, use)
            for i in range(use):
                ...
                extra = {
                    ...
                    "parent_episode_id": f"{terrain}|{seed}|{Path(paths[i]).name if i < len(paths) else i}|{i}",
                    "x": x_t[i].detach().cpu().clone(),
                    "state_kind": kind,
                }
...
def _roll_features(
    ...
    for t in range(horizon):
        ...
        e_use = eps
        if apply and eps_vec is not None:
            e_use = torch.zeros(n_envs, device=device, dtype=z_nom.dtype)
            e_use[:n_valid] = eps_vec
        z_exec = geodesic_z(z_nom, v_pad, e_use) if apply else z_nom
...
def _parent_ep(s: dict) -> str:
    return str(s.get("parent_episode_id") or str(s["episode_id"]).split("|p")[0])


@torch.inference_mode()
def _refresh_snaps(env, policy, runner, injector, snaps, vis_i, vis_flags, layout, pelvis_i, ankle_i):
    n_envs = int(env.unwrapped.num_envs)
    cmd = env.unwrapped.command_manager.get_term("motion")
    asset = env.unwrapped.scene["robot"]
    origins = env.unwrapped.scene.env_origins
    ...
    for start in range(0, len(snaps), n_envs):
        batch = snaps[start : start + n_envs]
        padded, n_valid = _pad_snaps(batch, n_envs)
        _restore_batch(env, padded)
        obs = injector.patch_policy_obs(env.get_observations()[0], env, "mapper")
        x, z = _extract_x(env, policy, runner, obs, vis_i, vis_flags, ankle_i, n_envs)
        yI = _y_intent(cmd, vis_i, vis_flags, origins)
        yB = _y_body(asset, cmd, layout, pelvis_i, ankle_i, n_envs)
        c2 = _contact2(env, ankle_i, n_envs)
        for i in range(n_valid):
            s = dict(batch[i])
            s["obs"] = obs[i].detach().cpu().clone()
            s["x"] = x[i].detach().cpu().clone()
            s["z_nom"] = z[i].detach().cpu().clone()
            s["y_I"] = yI[i].detach().cpu().clone()
            s["y_B"] = yB[i].detach().cpu().clone()
            s["contact"] = c2[i].detach().cpu().clone()
            out.append(s)
    return out


def _dump_dataset_b(path: Path, rows: list[dict], terrain: str, x_dim: int) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(
        path,
        x=np.stack([r["x"] for r in rows]).astype(np.float32),
        z0=np.stack([r["z0"] for r in rows]).astype(np.float32),
        C=np.stack([r["C"] for r in rows]).astype(np.float32),
        J_I=np.stack([r["J_I"] for r in rows]).astype(np.float32),
        episode_id=np.asarray([r["episode_id"] for r in rows]),
        parent_episode_id=np.asarray([r["parent_episode_id"] for r in rows]),
        window=np.asarray([r["window"] for r in rows]),
        state_kind=np.asarray([r["state_kind"] for r in rows]),
        terrain=np.asarray([r["terrain"] for r in rows]),
        seed=np.asarray([r["seed"] for r in rows], dtype=np.int32),
        t=np.asarray([r["t"] for r in rows], dtype=np.int32),
        clip=np.asarray([r["clip"] for r in rows]),
        e0=np.asarray([r["e0"] for r in rows], dtype=np.float32),
        x_dim=np.asarray(int(x_dim)),
        terrain_tag=np.asarray(terrain),
        held_out=np.asarray(terrain in HELD_OUT_TERRAINS),
        lambda_soft=np.asarray(1.0),
        no_terrain_in_model=np.asarray(True),
    )


def _row_from_matched(s: dict, rec: dict) -> dict:
    from p3_common import tilde_C_I

    z0 = np.asarray(rec["z0"], dtype=np.float64).reshape(-1)
    j_i = np.asarray(rec["J_I"], dtype=np.float64)
    x = s["x"].numpy().astype(np.float32).copy()
    x[:Z_DIM] = z0.astype(np.float32)
    return {
        "x": x,
        "z0": z0.astype(np.float32),
        "C": tilde_C_I(j_i, z0).astype(np.float32),
        "J_I": j_i.astype(np.float32),
        "episode_id": s["episode_id"],
        "parent_episode_id": _parent_ep(s),
        "window": s["window"],
        "state_kind": "recovery" if s["window"] == "recovery" else "nominal",
        "terrain": s["terrain"],
        "seed": int(s["seed"]),
        "t": int(s["t"]),
        "clip": s["clip"],
...
def _row_from_jac(s: dict, j_i: np.ndarray) -> dict:
    from p3_common import tilde_C_I

    z0 = s["z_nom"].numpy().astype(np.float64).reshape(-1)
    x = s["x"].numpy().astype(np.float32).copy()
    x[:Z_DIM] = z0.astype(np.float32)
    return {
        "x": x,
        "z0": z0.astype(np.float32),
        "C": tilde_C_I(j_i, z0).astype(np.float32),
        "J_I": np.asarray(j_i, dtype=np.float32),
        "episode_id": s["episode_id"],
        "parent_episode_id": _parent_ep(s),
        "window": s["window"],
        "state_kind": str(s.get("state_kind") or s["window"]),
        "terrain": s["terrain"],
        "seed": int(s["seed"]),
        "t": int(s["t"]),
        "clip": s["clip"],
...
def _run_collect_p3b(env_cfg, agent_cfg, resume_path, motion_dir, terrain, out_root: Path):
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    cell = out_root / "p3b_learned_projector" / "dataset_b"
    cell.mkdir(parents=True, exist_ok=True)
    out_p = cell / f"{terrain}.npz"
    meta_p = cell / f"{terrain}_meta.json"
    if out_p.exists():
        print(f"[p3b] skip collect {terrain}", flush=True)
        return json.loads(meta_p.read_text()) if meta_p.exists() else {}
    ifs = Path(args_cli.ifs_root)
    jac_p = ifs / "dataset_a" / terrain / "jac.npz"
    J_map = _load_jac_map(jac_p)
    ...
    all_snaps = []
    for seed in seeds:
        print(f"[p3b] ===== collect {terrain} seed={seed} =====", flush=True)
        torch.manual_seed(seed)
        np.random.seed(seed)
        snaps, pelvis_i, ankle_i, _use, _paths = _collect_snaps(
            env, policy, runner, injector, int(args_cli.steps), seed, terrain, vis_i, vis_flags, mask_name, layout
        ...
        all_snaps.extend(snaps)
    matched, nom_snaps, rec_snaps = [], [], []
    seen_keys = set()
    for s in all_snaps:
        key = (s["episode_id"], int(s["t"]), s["window"])
        if key not in J_map or key in seen_keys:
            continue
        seen_keys.add(key)
        row = _row_from_matched(s, J_map[key])
        matched.append(row)
        if s["window"] == "nominal":
            nom_snaps.append(s)
        elif s["window"] == "recovery":
            rec_snaps.append(s)
    rng = np.random.RandomState(2026 + (sum(ord(c) for c in terrain) % 997))
    max_nom = int(args_cli.max_nominal)
    max_off = int(args_cli.max_offman)
    if len(nom_snaps) > max_nom:
        idx = rng.choice(len(nom_snaps), max_nom, replace=False)
        keep = set(int(i) for i in idx.tolist())
        nom_snaps = [nom_snaps[i] for i in range(len(nom_snaps)) if i in keep]
        nom_eps = {(s["episode_id"], int(s["t"])) for s in nom_snaps}
        matched = [
            r for r in matched
            if r["state_kind"] != "nominal" or (r["episode_id"], int(r["t"])) in nom_eps
        ]
    n_rec = sum(1 for r in matched if r["state_kind"] == "recovery")
    need_pert = 0 if terrain in HELD_OUT_TERRAINS else max(0, max_off - n_rec)
    pert_src = nom_snaps[:]
    rng.shuffle(pert_src)
    pert_src = pert_src[: max(need_pert, 0)]
    print(
        f"[p3b] {terrain} matched={len(matched)} nom={len(nom_snaps)} rec={n_rec} pert={len(pert_src)}",
        flush=True,
    )
    pert_rows = []
    if pert_src:
        pert_snaps = [_perturb_snap(s, rng) for s in pert_src]
        kw = dict(vis_i=vis_i, vis_flags=vis_flags, layout=layout, pelvis_i=pelvis_i, ankle_i=ankle_i)
        pert_snaps = _refresh_snaps(env, policy, runner, injector, pert_snaps, **kw)
        jac_rows = _compute_jacobians(env, policy, runner, injector, pert_snaps, **kw)
        j_by_ep = {(r["episode_id"], int(r["t"])): r["J_I"] for r in jac_rows}
        for s in pert_snaps:
            j_i = j_by_ep.get((s["episode_id"], int(s["t"])))
            if j_i is None or not np.isfinite(j_i).all():
                continue
            pert_rows.append(_row_from_jac(s, j_i))
        if len(pert_rows) > need_pert:
            pert_rows = pert_rows[:need_pert]
    rows = matched + pert_rows
    if terrain in HELD_OUT_TERRAINS and len(rows) > 600:
        rng2 = np.random.RandomState(2026)
        idx = rng2.choice(len(rows), 600, replace=False)
        rows = [rows[i] for i in sorted(idx.tolist())]
    x_dim = int(rows[0]["x"].shape[0]) if rows else 0
    _dump_dataset_b(out_p, rows, terrain, x_dim)
    counts = {}
    for r in rows:
        counts[r["state_kind"]] = counts.get(r["state_kind"], 0) + 1
    meta = {
        "terrain": terrain,
        "held_out": terrain in HELD_OUT_TERRAINS,
        "n": len(rows),
    ...
    meta_p.write_text(json.dumps(_sanitize(meta), indent=2), encoding="utf-8")
    print(f"[p3b] wrote {out_p} n={len(rows)} counts={counts}", flush=True)
    try:
        env.close()
    except Exception:
        pass
...
def _load_selected_models(out_root: Path, device):
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from p3b_model import IntentMetricMLP

    sel_p = Path(args_cli.selected_json) if args_cli.selected_json else (
        out_root / "p3b_learned_projector" / "selected.json"
    )
    if not sel_p.exists():
        raise FileNotFoundError(f"need {sel_p}")
    sel = json.loads(sel_p.read_text())
    want = {m.strip() for m in str(args_cli.models).split(",") if m.strip()}
    nets = {}
    for name, rec in sel.items():
        if name not in want:
            continue
        ckpt = torch.load(rec["ckpt"], map_location="cpu", weights_only=False)
        in_dim = int(ckpt["in_dim"])
        out_dim = int(ckpt["out_dim"])
        net = IntentMetricMLP(in_dim, out_dim=out_dim)
        net.load_state_dict(ckpt["state_dict"])
        net.to(device)
        net.eval()
        nets[name] = {
            "net": net,
            "mode": ckpt.get("mode", "chol"),
            "rank": int(ckpt.get("rank") or 8),
            "x_mean": torch.as_tensor(ckpt["x_mean"], device=device, dtype=torch.float32),
            "x_std": torch.as_tensor(ckpt["x_std"], device=device, dtype=torch.float32),
            "in_dim": in_dim,
        }
    return nets
...
@torch.inference_mode()
def _pred_C(spec, x, z):
    from p3b_model import chol_to_C, lowrank_to_C

    xn = ((x - spec["x_mean"]) / spec["x_std"].clamp(min=1e-6)).clamp(-10.0, 10.0)
    if spec["in_dim"] == Z_DIM:
        raw = spec["net"](xn[:, :Z_DIM])
    else:
        raw = spec["net"](xn)
    if spec["mode"] == "lowrank":
        return lowrank_to_C(raw, z, spec["rank"])
    return chol_to_C(raw, z)


def _run_rollout_p3b(env_cfg, agent_cfg, resume_path, motion_dir, terrain, out_root: Path):
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    out_json = out_root / "p3b_learned_projector" / "rollout" / f"{terrain}.json"
    if out_json.exists():
        print(f"[p3b] skip rollout {terrain}", flush=True)
        return json.loads(out_json.read_text())
    ifs = Path(args_cli.ifs_root)
    snaps_p = ifs / "dataset_a" / terrain / "snaps_eval.pt"
    jac_p = ifs / "dataset_a" / terrain / "jac.npz"
    ...
    ankle_i = [names.index(n) for n in ANKLE_BODIES if n in names]
    device = env.unwrapped.device
    snaps = [s for s in torch.load(snaps_p, map_location="cpu", weights_only=False) if s.get("window") == "recovery"]
    J_map = _load_jac_map(jac_p)
    scales = _load_scales(ifs)
    pos_scale = float(scales.get("pos_scale", POS_SCALE))
    sig_b = torch.as_tensor(scales["sig_B"], device=device, dtype=torch.float32).clamp(min=1e-8)
    ...
    burst = int(args_cli.ucr_burst)
    horizon = int(args_cli.ucr_horizon)
    eps = math.radians(float(args_cli.ucr_deg))
    eps5 = math.radians(5.0)
    n_envs = int(env.unwrapped.num_envs)
    nets = _load_selected_models(out_root, device)
    methods = ["oracle"] + list(nets.keys())
    dstar_cache = {}
    cache_p = out_root / "p3a_oracle_projector" / f"{terrain}_dstars.npz"
    if cache_p.exists():
        zc = np.load(cache_p, allow_pickle=True)
    ...
    for start in range(0, len(snaps), n_envs):
        batch = snaps[start : start + n_envs]
        n_valid = len(batch)
        print(f"[p3b] rollout {terrain} {start}:{start+n_valid}/{len(snaps)}", flush=True)
        refreshed = _refresh_snaps(env, policy, runner, injector, batch, **kw)
        z0 = torch.stack([s["z_nom"] for s in refreshed], dim=0).to(device)
        x = torch.stack([s["x"] for s in refreshed], dim=0).to(device)
        J_list = []
        ok_j = True
        for s in batch:
            key = (s["episode_id"], int(s["t"]), s["window"])
            ...
            J_list.append(J_map[key]["J_I"])
        if not ok_j:
            print("[p3b] missing J_I, skip batch", flush=True)
            continue
        J_I = torch.as_tensor(np.stack(J_list), device=device, dtype=torch.float32)
        C_oracle = _tilde_C_torch(J_I, z0, pos_scale)
        d_star = torch.zeros(n_valid, Z_DIM, device=device)
        for i, s in enumerate(batch):
            key = (s["episode_id"], int(s["t"]))
            if key not in dstar_cache:
                print("[p3b] missing d*, skip batch", flush=True)
                ok_j = False
                break
            d_star[i] = torch.as_tensor(dstar_cache[key], device=device)
        if not ok_j:
        ...
        C_map = {"oracle": C_oracle}
        for name, spec in nets.items():
            C_map[name] = _pred_C(spec, x, z0)
        method_out = {}
        for name in methods:
            P = _soft_P_torch(C_map[name], 1.0, z0)
            d_exec, rd, supp = _apply_P_torch(P, d_star, z0)
            d_use = torch.where(supp.unsqueeze(-1), torch.zeros_like(d_exec), d_exec)
            _yIp, _yBp, eh, extra = _roll_features(
                env, policy, runner, injector, batch, n_valid,
                d_use, eps, horizon, **kw, burst=burst, record_e=True,
            )
            peak, integ = _di_from_evec(extra["e0_vec"], extra["evec"])
            yI5p, yB5p, _, _ = _roll_features(
                env, policy, runner, injector, batch, n_valid, d_exec, eps5, 1, **kw,
            )
            yI5m, yB5m, _, _ = _roll_features(
                env, policy, runner, injector, batch, n_valid, d_exec, -eps5, 1, **kw,
            )
            ...
            db5 = torch.linalg.norm((yB5p - yB5m) / 2.0 / sig_b.view(1, -1), dim=-1)
            A = (e_parent[:, -1] - eh[:, -1]).detach()
            ph = torch.bmm(P, d_star.unsqueeze(-1)).squeeze(-1)
            po = torch.bmm(_soft_P_torch(C_oracle, 1.0, z0), d_star.unsqueeze(-1)).squeeze(-1)
            cos = (ph * po).sum(-1) / (ph.norm(dim=-1) * po.norm(dim=-1) + 1e-8)
            method_out[name] = {
                "A": A.cpu().numpy(),
                "e_term": eh[:, -1].detach().cpu().numpy(),
                "e0": extra["e0"].cpu().numpy(),
                ...
                "Rd": rd.detach().cpu().numpy(),
                "suppressed": supp.detach().cpu().numpy().astype(np.int32),
                "cos_ucr": cos.detach().cpu().numpy(),
            }
            print(f"[p3b]   {name}", flush=True)
        for i in range(n_valid):
            s = batch[i]
            item = {
                "episode_id": s["episode_id"],
    ...
    payload = {
        ...
        "ucr_burst": burst,
        "ucr_horizon": horizon,
        "ucr_deg": float(args_cli.ucr_deg),
        "lambda_soft": 1.0,
        "methods": methods,
        "rows": rows,
        "no_terrain_in_model": True,
    ...
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(_sanitize(payload), indent=2), encoding="utf-8")
    print(f"[p3b] wrote {out_json} n={len(rows)}", flush=True)
    try:
        env.close()
    except Exception:
        pass
...
def _action_scale_diag(env, n_valid: int) -> torch.Tensor:
    """J_{q_target,a} = diag(scale). Decoder 29-D aligns with joint_pos term."""
    term = env.unwrapped.action_manager.get_term("joint_pos")
    scale = term._scale
    device = env.unwrapped.device
    if torch.is_tensor(scale):
        s = scale[:n_valid].to(device=device, dtype=torch.float32)
    else:
        dim = int(term.action_dim)
        s = torch.full((n_valid, dim), float(scale), device=device, dtype=torch.float32)
    return torch.diag_embed(s)


def _decoder_Ja_z(policy, z: torch.Tensor, proprio: torch.Tensor) -> torch.Tensor:
    """J_{a,z} via autograd. Must be outside inference_mode. z is ambient 16-D."""
    z = z.detach()
    ...
    with torch.enable_grad():
        for i in range(int(z.shape[0])):
            ...
            def _f(zz, _pi=pi):
                return policy.muse._decode(policy.muse._maybe_normalize_latent(zz), _pi)

            J = torch.autograd.functional.jacobian(_f, zi, create_graph=False)
            if J.dim() == 4:
                rows.append(J[0, :, 0, :])
            elif J.dim() == 3:
                rows.append(J[:, 0, :] if J.shape[1] == 1 else J[0])
            else:
                rows.append(J.reshape(-1, zi.shape[-1]))
    return torch.stack(rows, dim=0)


def _fk_JI_q(asset, vis_names: list[str], n_valid: int, n_act: int) -> torch.Tensor:
    """World-frame positional FK Jacobian of intent bodies wrt actuated joints."""
    jac = asset.root_physx_view.get_jacobians()
    names = list(asset.data.body_names)
    off = 0 if bool(asset.is_fixed_base) else 6
    cols = list(range(off, off + n_act))
    parts = []
    for bname in vis_names:
        bidx = names.index(bname)
        jidx = bidx - 1 if bool(asset.is_fixed_base) else bidx
        parts.append(jac[:n_valid, jidx, :3, cols].to(dtype=torch.float32))
    return torch.cat(parts, dim=1)


def _analytic_JI(env, policy, runner, injector, n_valid: int, vis_names: list[str]):
    """J_{I,z}^{ana} ≈ J_{I,q} J_{q,a} J_{a,z}. Instantaneous; no terrain/vision."""
    obs = injector.patch_policy_obs(env.get_observations()[0], env, "mapper")
    with torch.inference_mode():
        z, proprio, _enc = _z_and_proprio(policy, runner, obs)
    z = z[:n_valid].detach().clone()
    proprio = proprio[:n_valid].detach().clone()
    Ja = _decoder_Ja_z(policy, z, proprio)
    Jqa = _action_scale_diag(env, n_valid)
    n_act = int(Jqa.shape[-1])
    if int(Ja.shape[1]) != n_act:
        raise RuntimeError(f"decoder dim {Ja.shape[1]} != action dim {n_act}")
    JIq = _fk_JI_q(env.unwrapped.scene["robot"], vis_names, n_valid, n_act)
    JIz = torch.bmm(JIq, torch.bmm(Jqa, Ja))
    return JIz, Ja, JIq, z


def _method_ucr_pack(
    env, policy, runner, injector, batch, n_valid, d_star, C_use, C_gt, z0, P_or,
    burst, horizon, eps, eps5, pos_scale, sig_b, kw,
):
    P = _soft_P_torch(C_use, 1.0, z0)
    d_exec, rd, supp = _apply_P_torch(P, d_star, z0)
    d_use = torch.where(supp.unsqueeze(-1), torch.zeros_like(d_exec), d_exec)
    _yIp, _yBp, eh, extra = _roll_features(
        env, policy, runner, injector, batch, n_valid,
    ...
    di5 = torch.linalg.norm((yI5p - yI5m) / 2.0 / pos_scale, dim=-1)
    db5 = torch.linalg.norm((yB5p - yB5m) / 2.0 / sig_b.view(1, -1), dim=-1)
    d_n = F.normalize(d_exec, dim=-1, eps=1e-8)
    leak = torch.einsum("bi,bij,bj->b", d_n, C_gt, d_n)
    po = torch.bmm(P_or, d_star.unsqueeze(-1)).squeeze(-1)
    ph = torch.bmm(P, d_star.unsqueeze(-1)).squeeze(-1)
    cos = (ph * po).sum(-1) / (ph.norm(dim=-1) * po.norm(dim=-1) + 1e-8)
    return {
        "d_exec": d_exec,
...
def _run_b4b(env_cfg, agent_cfg, resume_path, motion_dir, terrain, out_root: Path):
    """P3-B4b: analytic decoder+FK Jacobian vs closed-loop oracle J_I. No training."""
    jac_p_out = out_root / "jac_ana" / f"{terrain}.npz"
    json_out = out_root / "rollout" / f"{terrain}.json"
    if jac_p_out.exists() and json_out.exists():
        print(f"[b4b] skip {terrain}", flush=True)
        return
    ifs = Path(args_cli.ifs_root)
    p3a = Path(args_cli.p3a_root)
    snaps_p = ifs / "dataset_a" / terrain / "snaps_eval.pt"
    jac_p = ifs / "dataset_a" / terrain / "jac.npz"
    env, runner, policy, injector = _make_env(env_cfg, agent_cfg, resume_path, motion_dir, terrain)
    cmd = env.unwrapped.command_manager.get_term("motion")
    ...
    mask_name = str(args_cli.mask_modes).split(",")[0].strip() or "torso"
    vis_flags = MASK_VIS.get(mask_name, (True, True, True))
    vis_names = [KP_VIS[k] for k, flag in enumerate(vis_flags) if flag]
    asset = env.unwrapped.scene["robot"]
    layout = _feature_layout(list(asset.data.joint_names))
    names = list(asset.data.body_names)
    pelvis_i = names.index("pelvis") if "pelvis" in names else None
    ankle_i = [names.index(n) for n in ANKLE_BODIES if n in names]
    device = env.unwrapped.device
    all_snaps = list(torch.load(snaps_p, map_location="cpu", weights_only=False))
    ...
    term = env.unwrapped.action_manager.get_term("joint_pos")
    print(
        f"[b4b] {terrain} action=JointPositionAction dim={term.action_dim} "
        f"fixed_base={bool(asset.is_fixed_base)} vis={vis_names} "
        f"n_all={len(all_snaps)} n_rec={len(rec_snaps)}",
        flush=True,
    )
    kw = dict(vis_i=vis_i, vis_flags=vis_flags, layout=layout, pelvis_i=pelvis_i, ankle_i=ankle_i)

    dump_rows = []
    for start in range(0, len(all_snaps), n_envs):
        batch = all_snaps[start : start + n_envs]
        n_valid = len(batch)
        padded, _ = _pad_snaps(batch, n_envs)
        _restore_batch(env, padded)
        env.unwrapped.scene.write_data_to_sim()
        env.unwrapped.sim.forward()
        J_ana, Ja, JIq, z_live = _analytic_JI(env, policy, runner, injector, n_valid, vis_names)
        if start == 0:
            sc = term._scale
            sc0 = float(sc[0].abs().mean().item()) if torch.is_tensor(sc) else float(sc)
            print(
                f"[b4b] J_ana {tuple(J_ana.shape)} ||J||={float(J_ana[0].norm().item()):.4g} "
                f"||Ja||={float(Ja[0].norm().item()):.4g} ||JIq||={float(JIq[0].norm().item()):.4g} "
                f"scale_mean={sc0:.4g}",
                flush=True,
            )
        for i in range(n_valid):
            s = batch[i]
            key = (s["episode_id"], int(s["t"]), s["window"])
            J_or = J_map[key]["J_I"] if key in J_map else None
            dump_rows.append(
                {
                    "z0": z_live[i].detach().cpu().numpy().astype(np.float32),
                    "J_I_ana": J_ana[i].detach().cpu().numpy().astype(np.float32),
                    "J_I_oracle": None if J_or is None else np.asarray(J_or, dtype=np.float32),
                    "episode_id": s["episode_id"],
                    "window": s["window"],
                    "terrain": s["terrain"],
                    "t": int(s["t"]),
        ...
        print(f"[b4b] jac {start}:{start + n_valid}/{len(all_snaps)}", flush=True)

    jac_p_out.parent.mkdir(parents=True, exist_ok=True)
    have_or = all(r["J_I_oracle"] is not None for r in dump_rows)
    np.savez_compressed(
        jac_p_out,
        z0=np.stack([r["z0"] for r in dump_rows]),
        J_I_ana=np.stack([r["J_I_ana"] for r in dump_rows]),
        J_I_oracle=np.stack([r["J_I_oracle"] for r in dump_rows]) if have_or else np.zeros((len(dump_rows), 1)),
        episode_id=np.asarray([r["episode_id"] for r in dump_rows]),
        window=np.asarray([r["window"] for r in dump_rows]),
        terrain=np.asarray([r["terrain"] for r in dump_rows]),
        t=np.asarray([r["t"] for r in dump_rows], dtype=np.int32),
        clip=np.asarray([r["clip"] for r in dump_rows]),
        e0=np.asarray([r["e0"] for r in dump_rows], dtype=np.float32),
        state_kind=np.asarray([r["state_kind"] for r in dump_rows]),
        pos_scale=np.asarray(pos_scale),
        action="joint_position_target",
        vis_names=np.asarray(vis_names),
        no_terrain_in_model=np.asarray(True),
        have_oracle=np.asarray(bool(have_or)),
    )
    print(f"[b4b] wrote {jac_p_out} n={len(dump_rows)}", flush=True)

    b1_path = args_cli.b1_ckpt or str(p3a / "p3b_learned_projector" / "checkpoints" / "B1_s2028.pt")
    spec = _load_b1(b1_path, device)
    if int(sig_b.numel()) == 1:
        sig_b = torch.ones(int(rec_snaps[0]["y_B"].shape[0]), device=device)
    burst = int(args_cli.ucr_burst)
    horizon = int(args_cli.ucr_horizon)
    eps = math.radians(float(args_cli.ucr_deg))
    eps5 = math.radians(5.0)
    dstar_cache = {}
    cache_p = p3a / "p3a_oracle_projector" / f"{terrain}_dstars.npz"
    if cache_p.exists():
        zc = np.load(cache_p, allow_pickle=True)
        for i, ep in enumerate(zc["episode_id"]):
            dstar_cache[(str(ep), int(zc["t"][i]))] = zc["d"][i]
    rows = []
    for start in range(0, len(rec_snaps), n_envs):
        batch = rec_snaps[start : start + n_envs]
        n_valid = len(batch)
        print(f"[b4b] ucr {terrain} {start}:{start + n_valid}/{len(rec_snaps)}", flush=True)
        refreshed = _refresh_snaps(env, policy, runner, injector, batch, **kw)
        z0 = torch.stack([s["z_nom"] for s in refreshed], dim=0).to(device)
        x = torch.stack([s["x"] for s in refreshed], dim=0).to(device)
        padded, _ = _pad_snaps(batch, n_envs)
        ...
        env.unwrapped.scene.write_data_to_sim()
        env.unwrapped.sim.forward()
        J_ana, _Ja, _JIq, _z = _analytic_JI(env, policy, runner, injector, n_valid, vis_names)
        J_list, ok_j = [], True
        for s in batch:
            key = (s["episode_id"], int(s["t"]), s["window"])
            if key not in J_map:
            ...
            J_list.append(J_map[key]["J_I"])
        if not ok_j:
            print("[b4b] missing oracle J_I, skip batch", flush=True)
            continue
        J_I = torch.as_tensor(np.stack(J_list), device=device, dtype=torch.float32)
        C_gt = _tilde_C_torch(J_I, z0, pos_scale)
        C_ana = _tilde_C_torch(J_ana, z0, pos_scale)
        C_hat = _pred_C(spec, x, z0)
        d_star = torch.zeros(n_valid, Z_DIM, device=device)
        for i, s in enumerate(batch):
            key = (s["episode_id"], int(s["t"]))
            if key not in dstar_cache:
                print("[b4b] missing d*, skip batch", flush=True)
                ok_j = False
                break
            d_star[i] = torch.as_tensor(dstar_cache[key], device=device)
        if not ok_j:
        ...
        P_or = _soft_P_torch(C_gt, 1.0, z0)
        packs = {}
        for name, C_use in (("oracle", C_gt), ("B1", C_hat), ("analytic", C_ana)):
            packs[name] = _method_ucr_pack(
                env, policy, runner, injector, batch, n_valid, d_star, C_use, C_gt, z0, P_or,
                burst, horizon, eps, eps5, pos_scale, sig_b, kw,
            )
        for i in range(n_valid):
            item = {
                "episode_id": batch[i]["episode_id"],
                "t": int(batch[i]["t"]),
                "terrain": terrain,
                "held_out": terrain in HELD_OUT_TERRAINS,
                "e_parent": float(e_parent[i, -1].item()),
            ...
            for name, mo in packs.items():
                A = (e_parent[:, -1] - mo["eh"][:, -1]).detach()
                item["methods"][name] = {
                    "A": float(A[i].item()),
                    "DI_peak": float(mo["DI_peak"][i].item()),
                    "DI_int": float(mo["DI_int"][i].item()),
                    "DI_5deg": float(mo["DI_5deg"][i].item()),
                    "DB_5deg": float(mo["DB_5deg"][i].item()),
                    "Rd": float(mo["rd"][i].item()),
                    "suppressed": int(mo["suppressed"][i].item()),
                    "cos_ucr": float(mo["cos"][i].item()),
                    "oracle_leak": float(mo["leak"][i].item()),
                }
            rows.append(item)
    payload = {
        "terrain": terrain,
        ...
        "n": len(rows),
        "action": "joint_position_target",
        "analytic": "J_I,z = J_I,q * diag(scale) * J_a,z",
        "no_terrain_in_model": True,
        "methods": ["oracle", "B1", "analytic"],
        "ucr_burst": burst,
        "ucr_horizon": horizon,
        "lambda_soft": 1.0,
        "rows": rows,
    ...
    json_out.write_text(json.dumps(_sanitize(payload), indent=2), encoding="utf-8")
    print(f"[b4b] wrote {json_out} n={len(rows)}", flush=True)
    try:
        env.close()
    except Exception:
        pass
...
def _load_b1(ckpt_path: str, device):
    from p3b_model import IntentMetricMLP
...
def _run_calib_p3b2(env_cfg, agent_cfg, resume_path, motion_dir, terrain, out_root: Path):
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    out_json = out_root / "calibration" / f"{terrain}.json"
    if out_json.exists():
        print(f"[p3b2] skip calib {terrain}", flush=True)
        return json.loads(out_json.read_text())
    ifs = Path(args_cli.ifs_root)
    p3a = Path(args_cli.p3a_root)
    snaps_p = ifs / "dataset_a" / terrain / "snaps_eval.pt"
    ...
    eps5 = math.radians(5.0)
    n_envs = int(env.unwrapped.num_envs)
    b1_path = args_cli.b1_ckpt or str(
        p3a / "p3b_learned_projector" / "checkpoints" / "B1_s2028.pt"
    )
    spec = _load_b1(b1_path, device)
    lams = [float(x) for x in str(args_cli.lambdas).split(",") if x.strip()]
    methods = [("oracle", 1.0, True)] + [(f"B1_{lam:g}", lam, False) for lam in lams]
    dstar_cache = {}
    cache_p = p3a / "p3a_oracle_projector" / f"{terrain}_dstars.npz"
    if cache_p.exists():
        zc = np.load(cache_p, allow_pickle=True)
    ...
    for start in range(0, len(snaps), n_envs):
        batch = snaps[start : start + n_envs]
        n_valid = len(batch)
        print(f"[p3b2] calib {terrain} {start}:{start+n_valid}/{len(snaps)}", flush=True)
        refreshed = _refresh_snaps(env, policy, runner, injector, batch, **kw)
        z0 = torch.stack([s["z_nom"] for s in refreshed], dim=0).to(device)
        x = torch.stack([s["x"] for s in refreshed], dim=0).to(device)
        J_list = []
        ...
        for s in batch:
            ...
            J_list.append(J_map[key]["J_I"])
        if not ok_j:
            print("[p3b2] missing J_I, skip batch", flush=True)
            continue
        J_I = torch.as_tensor(np.stack(J_list), device=device, dtype=torch.float32)
        C_gt = _tilde_C_torch(J_I, z0, pos_scale)
        C_hat = _pred_C(spec, x, z0)
        ...
        for i, s in enumerate(batch):
            key = (s["episode_id"], int(s["t"]))
            if key not in dstar_cache:
                print("[p3b2] missing d*, skip batch", flush=True)
                ok_j = False
                break
            d_star[i] = torch.as_tensor(dstar_cache[key], device=device)
        if not ok_j:
        ...
        P_or = _soft_P_torch(C_gt, 1.0, z0)
        method_out = {}
        for name, lam, is_oracle in methods:
            C_use = C_gt if is_oracle else C_hat
            P = _soft_P_torch(C_use, float(lam), z0)
            d_exec, rd, supp = _apply_P_torch(P, d_star, z0)
            d_use = torch.where(supp.unsqueeze(-1), torch.zeros_like(d_exec), d_exec)
            _yIp, _yBp, eh, extra = _roll_features(
                env, policy, runner, injector, batch, n_valid,
            ...
            method_out[name] = {
                ...
                "suppressed": supp.detach().cpu().numpy().astype(np.int32),
                "cos_ucr": cos.detach().cpu().numpy(),
                "oracle_leak": leak.detach().cpu().numpy(),
                "lambda": float(lam),
            }
            print(f"[p3b2]   {name}", flush=True)
        for i in range(n_valid):
            s = batch[i]
            item = {
                "episode_id": s["episode_id"],
            ...
            for name, _lam, _iso in methods:
                ...
                item["methods"][name] = {
                    ...
                    "suppressed": int(mo["suppressed"][i]),
                    "cos_ucr": float(mo["cos_ucr"][i]),
                    "oracle_leak": float(mo["oracle_leak"][i]),
                    "lambda": float(mo["lambda"]),
                    "dE": float(mo["e0"][i] - mo["e_term"][i]),
                }
            rows.append(item)
    payload = {
    ...
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(_sanitize(payload), indent=2), encoding="utf-8")
    print(f"[p3b2] wrote {out_json} n={len(rows)}", flush=True)
    try:
        env.close()
    except Exception:
        pass
...
def _run_shield_p3b2(env_cfg, agent_cfg, resume_path, motion_dir, terrain, out_root: Path):
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    out_json = out_root / "p3b2_intent_shield" / "rollout" / f"{terrain}.json"
    if out_json.exists():
        print(f"[b21] skip shield {terrain}", flush=True)
        return json.loads(out_json.read_text())
    ifs = Path(args_cli.ifs_root)
    p3a = Path(args_cli.p3a_root)
    snaps_p = ifs / "dataset_a" / terrain / "snaps_eval.pt"
    ...
    n_envs = int(env.unwrapped.num_envs)
    b1_path = args_cli.b1_ckpt or str(p3a / "p3b_learned_projector" / "checkpoints" / "B1_s2028.pt")
    b21_path = args_cli.b21_ckpt
    if not b21_path:
        raise FileNotFoundError("--b21_ckpt required for shield")
    spec_b1 = _load_b1(b1_path, device)
    spec_b21 = _load_b1(b21_path, device)
    methods = [("oracle", True, None, 1.0), ("B1", False, spec_b1, 1.0), ("B21", False, spec_b21, 1.0)]
    dstar_cache = {}
    cache_p = p3a / "p3a_oracle_projector" / f"{terrain}_dstars.npz"
    if cache_p.exists():
        zc = np.load(cache_p, allow_pickle=True)
    ...
    for start in range(0, len(snaps), n_envs):
        batch = snaps[start : start + n_envs]
        n_valid = len(batch)
        print(f"[b21] shield {terrain} {start}:{start+n_valid}/{len(snaps)}", flush=True)
        refreshed = _refresh_snaps(env, policy, runner, injector, batch, **kw)
        z0 = torch.stack([s["z_nom"] for s in refreshed], dim=0).to(device)
        x = torch.stack([s["x"] for s in refreshed], dim=0).to(device)
        J_list = []
        ...
        for s in batch:
            ...
            J_list.append(J_map[key]["J_I"])
        if not ok_j:
            print("[b21] missing J_I, skip batch", flush=True)
            continue
        J_I = torch.as_tensor(np.stack(J_list), device=device, dtype=torch.float32)
        C_gt = _tilde_C_torch(J_I, z0, pos_scale)
        d_star = torch.zeros(n_valid, Z_DIM, device=device)
        for i, s in enumerate(batch):
            key = (s["episode_id"], int(s["t"]))
            if key not in dstar_cache:
                print("[b21] missing d*, skip batch", flush=True)
                ok_j = False
                break
            d_star[i] = torch.as_tensor(dstar_cache[key], device=device)
        if not ok_j:
        ...
        P_or = _soft_P_torch(C_gt, 1.0, z0)
        method_out = {}
        for name, is_oracle, spec, lam in methods:
            C_use = C_gt if is_oracle else _pred_C(spec, x, z0)
            P = _soft_P_torch(C_use, float(lam), z0)
            d_exec, rd, supp = _apply_P_torch(P, d_star, z0)
            d_use = torch.where(supp.unsqueeze(-1), torch.zeros_like(d_exec), d_exec)
            _yIp, _yBp, eh, extra = _roll_features(
    ...
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(_sanitize(payload), indent=2), encoding="utf-8")
    print(f"[b21] wrote {out_json} n={len(rows)}", flush=True)
    try:
        env.close()
    except Exception:
        pass
...
def _run_b3_rollout(env_cfg, agent_cfg, resume_path, motion_dir, terrain, out_root: Path):
    """P3-B3 UCR burst for R2 ensemble-mean, R3 UCB attenuation, R4 worst-case member."""
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    b3_root = Path(args_cli.b3_out)
    out_json = b3_root / "rollout" / f"{terrain}.json"
    if out_json.exists():
        print(f"[p3b3] skip {terrain}", flush=True)
        return json.loads(out_json.read_text())
    ifs = Path(args_cli.ifs_root)
    p3a = Path(args_cli.p3a_root)
    snaps_p = ifs / "dataset_a" / terrain / "snaps_eval.pt"
    ...
    eps5 = math.radians(5.0)
    n_envs = int(env.unwrapped.num_envs)
    beta = float(args_cli.ucb_beta)
    ckpt_s = str(args_cli.ensemble_ckpts).strip()
    if ckpt_s:
        ckpts = [p.strip() for p in ckpt_s.split(",") if p.strip()]
    else:
        cdir = p3a / "p3b_learned_projector" / "checkpoints"
        ckpts = [str(cdir / f"B1_s{s}.pt") for s in (2026, 2027, 2028)]
    specs = [_load_b1(p, device) for p in ckpts]
    print(f"[p3b3] ensemble n={len(specs)} beta={beta} {terrain}", flush=True)
    dstar_cache = {}
    cache_p = p3a / "p3a_oracle_projector" / f"{terrain}_dstars.npz"
    if cache_p.exists():
        zc = np.load(cache_p, allow_pickle=True)
    ...
    def _pack(A, eh, extra, di5, db5, rd, supp, leak, cos, alpha=None, mstar=None):
        peak, integ = _di_from_evec(extra["e0_vec"], extra["evec"])
        out = {
            "A": A.cpu().numpy(),
            "e_term": eh[:, -1].detach().cpu().numpy(),
        ...
        if alpha is not None:
            out["alpha"] = alpha.detach().cpu().numpy()
        if mstar is not None:
            out["mstar"] = mstar.detach().cpu().numpy().astype(np.int32)
        return out

    for start in range(0, len(snaps), n_envs):
        batch = snaps[start : start + n_envs]
        n_valid = len(batch)
        print(f"[p3b3] {terrain} {start}:{start + n_valid}/{len(snaps)}", flush=True)
        refreshed = _refresh_snaps(env, policy, runner, injector, batch, **kw)
        z0 = torch.stack([s["z_nom"] for s in refreshed], dim=0).to(device)
        x = torch.stack([s["x"] for s in refreshed], dim=0).to(device)
        J_list = []
        ...
        for s in batch:
            ...
            J_list.append(J_map[key]["J_I"])
        if not ok_j:
            print("[p3b3] missing J_I, skip batch", flush=True)
            continue
        J_I = torch.as_tensor(np.stack(J_list), device=device, dtype=torch.float32)
        C_gt = _tilde_C_torch(J_I, z0, pos_scale)
        d_star = torch.zeros(n_valid, Z_DIM, device=device)
        for i, s in enumerate(batch):
            key = (s["episode_id"], int(s["t"]))
            if key not in dstar_cache:
                print("[p3b3] missing d*, skip batch", flush=True)
                ok_j = False
                break
            d_star[i] = torch.as_tensor(dstar_cache[key], device=device)
        if not ok_j:
        ...
        C_stack = torch.stack([_pred_C(sp, x, z0) for sp in specs], dim=0)
        C_bar = C_stack.mean(0)
        P_or = _soft_P_torch(C_gt, 1.0, z0)
        P_mu = _soft_P_torch(C_bar, 1.0, z0)
        dT = _tan_raw(d_star, z0)
        s_m = torch.einsum("mbij,bj,bi->mb", C_stack, dT, dT)
        mu = s_m.mean(0)
        sig = s_m.std(0, unbiased=True) if s_m.shape[0] > 1 else torch.zeros_like(mu)
        alpha = 1.0 / (1.0 + mu + beta * sig)
        mstar = s_m.argmax(0)
        idx = torch.arange(n_valid, device=device)
        C_wc = C_stack[mstar, idx]
        P_wc = _soft_P_torch(C_wc, 1.0, z0)
        d_mu, rd_mu, supp_mu = _apply_P_torch(P_mu, d_star, z0)
        d_wc, rd_wc, supp_wc = _apply_P_torch(P_wc, d_star, z0)
        d_mu_use = torch.where(supp_mu.unsqueeze(-1), torch.zeros_like(d_mu), d_mu)
        d_wc_use = torch.where(supp_wc.unsqueeze(-1), torch.zeros_like(d_wc), d_wc)
        method_out = {}
        jobs = [
            ("R2_mean", d_mu_use, d_mu, rd_mu, supp_mu, P_mu, eps5, None),
            ("R3_ucb", d_mu_use, d_mu, rd_mu, supp_mu, P_mu, None, eps5 * alpha),
            ("R4_worst", d_wc_use, d_wc, rd_wc, supp_wc, P_wc, eps5, None),
        ]
        for name, d_burst, d_dir, rd, supp, P, eps_s, eps_v in jobs:
            _yIp, _yBp, eh, extra = _roll_features(
                env, policy, runner, injector, batch, n_valid,
                d_burst, float(eps_s) if eps_s is not None else 0.0, horizon, **kw,
                burst=burst, record_e=True, eps_vec=eps_v,
            )
            yI5p, yB5p, _, _ = _roll_features(
                env, policy, runner, injector, batch, n_valid, d_dir, eps5, 1, **kw,
            )
            yI5m, yB5m, _, _ = _roll_features(
                env, policy, runner, injector, batch, n_valid, d_dir, -eps5, 1, **kw,
            )
            ...
            db5 = torch.linalg.norm((yB5p - yB5m) / 2.0 / sig_b.view(1, -1), dim=-1)
            A = (e_parent[:, -1] - eh[:, -1]).detach()
            d_n = F.normalize(d_dir, dim=-1, eps=1e-8)
            leak = torch.einsum("bi,bij,bj->b", d_n, C_gt, d_n)
            ph = torch.bmm(P, d_star.unsqueeze(-1)).squeeze(-1)
            po = torch.bmm(P_or, d_star.unsqueeze(-1)).squeeze(-1)
            cos = (ph * po).sum(-1) / (ph.norm(dim=-1) * po.norm(dim=-1) + 1e-8)
            method_out[name] = _pack(
                A, eh, extra, di5, db5, rd, supp, leak, cos,
                alpha=alpha if name == "R3_ucb" else None,
                mstar=mstar if name == "R4_worst" else None,
            )
            print(f"[p3b3]   {name}", flush=True)
        for i in range(n_valid):
            s = batch[i]
            item = {
                "episode_id": s["episode_id"],
            ...
            for name in ("R2_mean", "R3_ucb", "R4_worst"):
                mo = method_out[name]
                rec = {
                    "A": float(mo["A"][i]),
                    "e_term": float(mo["e_term"][i]),
                ...
                if "alpha" in mo:
                    rec["alpha"] = float(mo["alpha"][i])
                if "mstar" in mo:
                    rec["mstar"] = int(mo["mstar"][i])
    ...
    payload = {
        ...
        "ucb_beta": beta,
        "ensemble": ckpts,
        "methods": ["R2_mean", "R3_ucb", "R4_worst"],
        "rows": rows,
        "no_terrain_in_model": True,
        "A_def": "E_parent - E_pert at t+0.5s, positive is better",
        "R3": "Version A magnitude attenuation: geodesic angle = 5deg * alpha(d*)",
    }
    out_json.parent.mkdir(parents=True, exist_ok=True)
    out_json.write_text(json.dumps(_sanitize(payload), indent=2), encoding="utf-8")
    print(f"[p3b3] wrote {out_json} n={len(rows)}", flush=True)
    try:
        env.close()
    except Exception:
        pass
...
def main(env_cfg, agent_cfg):
    ...
    print(f"[p3b2] ckpt {resume_path} stage={args_cli.stage}", flush=True)
    ...
    for terrain in terrains:
        if args_cli.stage == "shield":
            _run_shield_p3b2(env_cfg, agent_cfg, resume_path, args_cli.motion, terrain, out_dir)
        elif args_cli.stage == "b3":
            _run_b3_rollout(env_cfg, agent_cfg, resume_path, args_cli.motion, terrain, out_dir)
        elif args_cli.stage == "b4b":
            _run_b4b(env_cfg, agent_cfg, resume_path, args_cli.motion, terrain, out_dir)
        else:
            _run_calib_p3b2(env_cfg, agent_cfg, resume_path, args_cli.motion, terrain, out_dir)
    print(f"[p3b2] {args_cli.stage} done", flush=True)
