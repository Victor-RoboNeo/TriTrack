#!/usr/bin/env python3
"""Intent-preserving latent freedom: clone-based Stage-2 diagnostic.

Frozen Mapper-B + Stage-2. Terrain never enters observations or the latent
basis. Geodesic perturbations on the unit sphere. No PPO / residual / GRU.
"""
...
OUT_DEFAULT = "results/intent_free_space"
...
parser = argparse.ArgumentParser(description="Intent-free latent space diagnostic. Frozen Stage-2.")
...
parser.add_argument("--stage", type=str, default="collect_jac", choices=("collect_jac", "eval", "all"))
...
def _roll_features(
    ...
    return yI[:n_valid].detach(), yB[:n_valid].detach(), ei[:n_valid].detach() if ei is not None else None
...
def _run_eval(env_cfg, agent_cfg, resume_path, motion_dir, terrain, out_root: Path):
    cell = out_root / "dataset_a" / terrain
    out_eval = out_root / "eval" / f"{terrain}.json"
    if out_eval.exists():
        print(f"[ifs] skip eval {terrain}", flush=True)
        return json.loads(out_eval.read_text())
    snaps_p = cell / "snaps_eval.pt"
    jac_p = cell / "jac.npz"
    if not snaps_p.exists():
        raise FileNotFoundError(f"missing {snaps_p}; run collect_jac first")
    env, runner, policy, injector = _make_env(env_cfg, agent_cfg, resume_path, motion_dir, terrain)
    cmd = env.unwrapped.command_manager.get_term("motion")
    body_names = list(cmd.cfg.body_names)
    vis_i = [body_names.index(n) for n in KP_VIS]
    ...
    ankle_i = [names.index(n) for n in ANKLE_BODIES if n in names]
    device = env.unwrapped.device
    snaps = torch.load(snaps_p, map_location="cpu")
    J_map = _load_jac_map(jac_p)
    scales = _load_scales(out_root)
    if int(np.asarray(scales["sig_B"]).size) == 1:
        nb = int(snaps[0]["y_B"].shape[0]) if snaps else 1
        scales["sig_B"] = np.ones(nb, dtype=np.float32)
        scales["mu_B"] = np.zeros(nb, dtype=np.float32)
    bases = _load_bases(out_root, int(args_cli.k_max), device)
    if "S2_free" not in bases:
        raise FileNotFoundError("missing basis/B_free.npz; run analyze --basis first")
    local_snaps = [s for s in snaps if s.get("split") in ("val", "test") and s["window"] == "nominal"]
    rec_snaps = [s for s in snaps if s["window"] == "recovery"]
    space_rows = _eval_spaces(
        env, policy, runner, injector, local_snaps, vis_i, vis_flags, layout, pelvis_i, ankle_i, bases, J_map, scales
    )
    ucr_rows, e_curves = _eval_ucr_proj(
        env, policy, runner, injector, rec_snaps, vis_i, vis_flags, layout, pelvis_i, ankle_i, bases, J_map, scales
    )
    payload = {
        "terrain": terrain,
        "held_out": terrain in HELD_OUT_TERRAINS,
        "n_local": len(space_rows),
        "n_ucr": len(ucr_rows),
        "k_max": int(args_cli.k_max),
        "eval_deg": [float(x) for x in str(args_cli.eval_deg).split(",") if x.strip()],
        "ucr_burst": int(args_cli.ucr_burst),
        "ucr_horizon": int(args_cli.ucr_horizon),
    ...
    out_eval.write_text(json.dumps(_sanitize(payload), indent=2), encoding="utf-8")
    ucr_dir = out_root / "ucr_projection"
    ucr_dir.mkdir(parents=True, exist_ok=True)
    if e_curves:
        np.savez_compressed(ucr_dir / f"{terrain}_e_hist.npz", **{k: v for k, v in e_curves.items()})
    print(f"[ifs] wrote {out_eval} n_local={len(space_rows)} n_ucr={len(ucr_rows)}", flush=True)
    try:
        env.close()
    except Exception:
        pass
...
def main(env_cfg, agent_cfg):
    ...
    print(f"[ifs] ckpt {resume_path} stage={args_cli.stage}", flush=True)
    ...
    for terrain in terrains:
        if args_cli.stage in ("collect_jac", "all"):
            _run_collect_jac(env_cfg, agent_cfg, resume_path, args_cli.motion, terrain, out_dir)
        if args_cli.stage in ("eval", "all"):
            _run_eval(env_cfg, agent_cfg, resume_path, args_cli.motion, terrain, out_dir)
    print("[ifs] done", flush=True)
