#!/usr/bin/env bash
# P2-D: frozen nominal controller + evidence-gated scan-only residual from model_50000.
# Does NOT train g_φ / Mapper-B / Stage-2. No parent L2, elastic, ori hinge, feedback spring.
# Authority: analytic α(H) with α(0)=0, r_max=tan(10°), L_zero + L_calm.
set -euo pipefail
...
RUN_NAME="${RUN_NAME:-tritrack_p2d_gated_from50000}"
MASTER_PORT="${MASTER_PORT:-29634}"
MAX_ITERS="${MAX_ITERS:-8000}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-g1_headhands_p2d_gated}"

mkdir -p "$ROOT/logs/tritrack"
LOG="$ROOT/logs/tritrack/p2d_train.log"

echo "[p2d] GPUs=$CUDA_VISIBLE_DEVICES nproc=$NPROC envs/rank=$NUM_ENVS"
echo "[p2d] resume=$RESUME_CHECKPOINT"
echo "[p2d] experiment=$EXPERIMENT_NAME run_name=$RUN_NAME iters=+$MAX_ITERS"
echo "[p2d] frozen g_phi + Mapper-B + Stage-2; train h_eta(H) + critic only"
echo "[p2d] scan 187  r_max=tan10°=0.176  gate α(0)=0  λ0=1 λc=0.5"
echo "[p2d] lr=5e-5 clip=0.1 desired_kl=0.005"
echo "[p2d] log=$LOG"
...
HYDRA_FULL_ERROR=1 torchrun --standalone --nnodes=1 --nproc_per_node="$NPROC" --master_port="$MASTER_PORT" \
    ...
    agent.policy.terrain_r_max=0.1763 \
    agent.policy.terrain_scan_zero=false \
    agent.policy.terrain_gate=true \
    agent.policy.terrain_gate_w_s=1.0 \
    agent.policy.terrain_gate_w_r=2.5 \
    agent.policy.terrain_gate_s0=0.06 \
    agent.policy.terrain_gate_tau=0.025 \
    agent.policy.terrain_gate_s_dead=0.02 \
    ...
    agent.algorithm.terrain_zero_coef=1.0 \
    agent.algorithm.terrain_calm_coef=0.5 \
