#!/usr/bin/env bash
# P2-A: conservative parent-anchored refinement from model_50000.
# Unchanged vs failed P2: terrain mix, task mix, rewards, lr=5e-5, Stage-2/Mapper frozen.
# Changed: PPO clip 0.1, desired_kl 0.005, parent g_phi L2 on Flat+Light only.
# Do NOT resume 50500. Do NOT change orientation reward this round.
set -euo pipefail
...
RUN_NAME="${RUN_NAME:-tritrack_p2a_parent_anchor_from50000}"
...
LOG="$ROOT/logs/tritrack/p2a_train.log"

echo "[p2a] GPUs=$CUDA_VISIBLE_DEVICES nproc=$NPROC envs/rank=$NUM_ENVS"
echo "[p2a] resume=$RESUME_CHECKPOINT"
echo "[p2a] experiment=$EXPERIMENT_NAME run_name=$RUN_NAME iters=+$MAX_ITERS"
echo "[p2a] PPO clip=0.1 desired_kl=0.005 parent_anchor=1.0 save_interval=250 lr=5e-5"
echo "[p2a] log=$LOG"
...
HYDRA_FULL_ERROR=1 torchrun --standalone --nnodes=1 --nproc_per_node="$NPROC" --master_port="$MASTER_PORT" \
    ...
    agent.algorithm.parent_residual_anchor_coef=1.0 \
